            ...
            match &stmt.kind {
                ...
                StatementKind::Call { name, args: _ } => {
                    ...
                    if let Some(body) = self.groups.get(name).cloned() {
                        ...
                        println!("⚠️  Warning: Group '{}' not found", name);
                    }
                }
        ...
        for event in &self.events.events {
            match event {
                crate::engine::audio::events::AudioEvent::Note {
                    ...
                    let synth_def = self.events.get_synth(synth_id).cloned().unwrap_or_default();

                    let mut params = SynthParams {
                        waveform: synth_def.waveform,
                ...
                crate::engine::audio::events::AudioEvent::Sample {
                    ...
                    // Get sample PCM data from registry (WASM feature only)
                    #[cfg(target_arch = "wasm32")]
                    {
                    ...
                    // For non-WASM builds, samples are not supported
                    #[cfg(not(target_arch = "wasm32"))]
                    {
                        let _ = (start_time, velocity); // Silence unused warnings on non-WASM targets
                        println!("⚠️  Sample playback not supported in native build: {}", uri);
                    }
                }
            }
        }
