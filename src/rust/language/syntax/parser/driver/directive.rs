...
pub fn parse_directive(line: &str, line_number: usize, file_path: &std::path::Path) -> Result<Statement> {
    ...
    if !line.starts_with("@load") {
        ...
        if line.starts_with("@import") {
            ...
            if let Some(open) = rest.find('{') {
                if let Some(close) = rest.find('}') {
                    let names = rest[open+1..close].split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect::<Vec<_>>();
                    ...
                    if after.starts_with(from_prefix) {
                        ...
                        return Ok(Statement::new(StatementKind::Import { names, source: path }, Value::Null, 0, line_number, 1));
                    }
                }
            }
        }
        ...
        if line.starts_with("@export") {
            ...
            if let Some(open) = rest.find('{') {
                if let Some(close) = rest.find('}') {
                    let names = rest[open+1..close].split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect::<Vec<_>>();
                    return Ok(Statement::new(StatementKind::Export { names, source: String::new() }, Value::Null, 0, line_number, 1));
                }
            }
        }
...
fn parse_use_directive(line: &str, line_number: usize) -> Result<Statement> {
    ...
    if rest.is_empty() {
        return Err(anyhow!("@use directive requires a plugin reference (author.plugin)"));
    }
    ...
    if plugin_parts.len() != 2 {
        return Err(anyhow!("@use directive requires format: author.plugin (got: {})", plugin_ref));
    }
