...
/// Parse directive keywords (import, export, use, load) without "@" prefix
pub fn parse_directive_keyword(
    line: &str,
    keyword: &str,
    line_number: usize,
    ...
    match keyword {
        "use" => parse_use_directive(line, line_number),
        "load" => parse_load_directive(line, line_number, file_path),
        "import" => parse_import_directive(line, line_number, file_path),
        "export" => parse_export_directive(line, line_number),
        _ => Err(anyhow!("Unknown directive: {}", keyword)),
    }
}
...
fn parse_import_directive(
    ...
    Err(anyhow!(
        "Invalid import syntax. Use: import {{ a, b }} from \"path\""
    ))
}

fn parse_export_directive(line: &str, line_number: usize) -> Result<Statement> {
    // syntax: export { a, b }
    ...
    Err(anyhow!("Invalid export syntax. Use: export {{ a, b }}"))
}

fn parse_load_directive(
    line: &str,
    line_number: usize,
    file_path: &std::path::Path,
) -> Result<Statement> {
...
// Old @ prefix support kept for backwards compatibility, but deprecated
pub fn parse_directive(
    ...
    eprintln!(
        "⚠️  WARNING: @ prefix for directives is deprecated. Use 'import', 'export', 'use', 'load' instead."
    );
